...
|-------------|----------|------|---------|
| 1 | NE corner, 3' high | 4" conduit | Sub signal cables (4x RCA from X6800H to ADAU1452, 8x to 1200AS2) |
...
| Contents | X6800H, ATI AT525NC, 4x 1200AS2 (+ 1 spare), ADAU1452, UPS, network |
...
| Parameter | Value |
|-----------|-------|
| Feed to garage | #2 Aluminum, 240V, 60A breaker at main house panel |
| Garage subpanel | Existing panel (size TBD — verify in field) |
| Existing load | Pool: Pentair IntelliFlow VSF + gas heater on 30A/240V breaker |
| Pool actual draw | ~3-5A @ 240V typical (variable speed, 200-900W depending on RPM) |
| Available for theater | ~55A @ 240V after pool (60A feed - ~5A pool typical) |
| Post-build estimated load | ~30-35A @ 240V equivalent (theater + HVAC) |
| Remaining headroom | ~20-25A |

**Critical note:** The existing 30A/240V pool breaker is massively oversized for the IntelliFlow VSF. The pump draws 3-5A typical at normal speed (200-900W). The 30A breaker reflects the pump's locked-rotor / startup current rating, not steady-state draw. The gas heater uses no electrical power beyond ignition controls (~100W). This means ~25A of that 30A breaker capacity is unused during normal operation.

**60A feed capacity assessment:**
- 60A × 240V = 14,400W total capacity
- Pool (typical): ~900W = 3.75A
- Theater (peak): ~3,900W = 16.3A (see load analysis below)
- HVAC: ~2,400W = 10A
- Total peak: ~30A — well within 60A feed
- NEC 80% continuous rule (48A): still within limits even at sustained peak

### Circuit Schedule

| Circuit | Type | Load | Est. Draw | Wire | Cost |
|---------|------|------|-----------|------|------|
| POOL (existing) | 30A/240V | Pentair IntelliFlow VSF + gas heater | ~5A typical, 16A startup | Existing | $0 |
...
| C | 20A/120V | X6800H + ATI + 2x APM2 + sources | ~500W | 12 AWG | $400 |
| D | 20A/120V | Projector (ceiling run) | ~350W | 12 AWG | $400 |
| F | 20A/120V | Gym | Variable | 12 AWG | $400 |

**Load analysis (theater only, excluding pool):**
- Idle: HVAC ~10A@240V + Subs ~0.75A@240V + Processor ~4.2A@120V + Projector ~3A@120V = ~17A @240V equivalent
- Peak (movie climax): Subs burst to ~2kW (~8.3A@240V), processor to ~800W, projector same = ~25A @240V equivalent
- With pool running simultaneously (worst case): 25A theater + 5A pool = 30A — 50% of 60A feed
- **Well within 60A service. No upgrade needed.**
